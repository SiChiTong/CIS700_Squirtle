...
class Demo_voice_command:
    ...
    def __init__(self):
        ...
        self.recognizer_sub = rospy.Subscriber('/recognizer/output',String,self.receive_speech_callback)
    ...
    def state_switcher(self,msg,command):
        ...
        if self.current_state == "free":
            ...
            if command == "forward":
                ...
                self.say(['Yes, sir, preparing for frontier exploration'])
                self.say(['loading dependencies'])
                
                # load frontier exploration including: lidar, mapping, move_base ,exploration planner
                os.system("roslaunch squirtle_frontier_exploration exploration_bringup_with_hokuyo.launch &")

                self.say(['checking dependency status'])

                rospy.wait_for_service("/hokuyo/get_loggers")
                self.say(['hokuyo lidar driver has been loaded'])

                rospy.wait_for_service("/hector_mapping/get_loggers")
                self.say(['hector mapping has been loaded'])

                rospy.wait_for_service("/move_base/get_loggers")
                self.say(['move base has been loaded'])

                rospy.wait_for_service("/get_exploration_path")
                self.say(['exploration planner has been loaded'])

                self.say(['dependencies status checking complete'])

                # unregister subscriber, since the subscriber callback will invalidate the next block of code
                self.recognizer_sub.unregister()

                # wait until user says yes or let's begin
                received_msg = None
                while received_msg is None or (received_msg.data != "let's begin" and received_msg.data != "yes"):
                    self.say(['should we begin, sir'])
                    try:
                        received_msg = rospy.wait_for_message("/recognizer/output",String)
                    except Exception, e:
                        self.say(['something is wrong with my recognizer, sir, return to free state'])
                        self.current_state = self.last_state
                        break
                ...
                # user should manually kill this demo,sorry
                self.current_state = self.last_state
